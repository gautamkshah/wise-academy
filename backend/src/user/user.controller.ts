...
export class UserController {
    ...
    @Get()
    async findAll(@Query('college') college?: string, @Query('role') role?: string) {
        // Build where clause
        const where: any = {};
        if (college) {
    ...
    @UseGuards(AuthGuard)
    @Post('handles')
    async updateHandles(@Body() handles: { leetcode_id?: string; codeforces_id?: string; codechef_id?: string; github_id?: string; atcoder_id?: string; hackerrank_id?: string }, @Req() req) {
    ...
    @UseGuards(AuthGuard)
    @Post('profile')
    async updateProfile(@Body() data: { name?: string; college?: string; branch?: string; year?: number; roll_no?: string; phone?: string }, @Req() req) {
        const userId = req.user.uid;
