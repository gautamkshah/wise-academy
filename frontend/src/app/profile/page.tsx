        ...
        <div className="min-h-screen bg-gray-900 text-white flex flex-col">
            ...
            <main className="flex-grow container mx-auto px-6 py-8 md:py-12">
                <div className="max-w-4xl mx-auto">
                    ...
                    {/* Header Card */}
                    <div className="bg-gray-800/50 p-6 md:p-8 rounded-3xl border border-gray-700 mb-8 flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8 relative overflow-hidden">
                        ...
                        <div className="w-24 md:w-32 h-24 md:h-32 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-3xl md:text-4xl font-bold shadow-2xl relative z-10 transition-transform hover:scale-105">
                            {profileUser?.name?.charAt(0) || user?.displayName?.charAt(0) || 'U'}
                        </div>

                        <div className="text-center md:text-left relative z-10 flex-grow">
                            <h1 className="text-2xl md:text-3xl font-bold mb-1 md:mb-2">{profileUser?.name || user?.displayName}</h1>
                            <p className="text-gray-400 text-sm md:text-base mb-4">{profileUser?.email || user?.email}</p>
                            <div className="flex flex-wrap justify-center md:justify-start gap-2 md:gap-3">
                                <span className="bg-blue-500/10 text-blue-400 px-3 md:px-4 py-1 rounded-full text-xs md:text-sm font-bold border border-blue-500/20">
                                    {profileUser?.role || 'Student'}
                                </span>
                                {profileUser?.college && <span className="bg-gray-700/50 text-gray-300 px-3 md:px-4 py-1 rounded-full text-xs md:text-sm border border-gray-600/30">{profileUser?.college}</span>}
                            </div>
                        </div>

                        <button
                            onClick={() => setIsEditModalOpen(true)}
                            className="w-full md:w-auto bg-gray-700 hover:bg-gray-600 text-white px-6 py-2.5 rounded-xl font-bold transition flex items-center justify-center gap-2 relative z-10"
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                        {/* Personal Info */}
                        <div className="bg-gray-800/30 p-6 md:p-8 rounded-3xl border border-gray-700">
