...
export default function DashboardPage() {
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white flex flex-col font-sans">
            ...
            <main className="flex-grow container mx-auto px-4 md:px-6 py-6 md:py-8 transition-all">

                {/* 1. Header Section */}
                <div className="flex flex-col md:flex-row justify-between items-center bg-gray-800/20 p-6 md:p-8 rounded-[2rem] border border-gray-800/50 mb-8 gap-6">
                    <div className="flex flex-col md:flex-row items-center gap-4 text-center md:text-left">
                        <div className="w-20 h-20 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-3xl font-bold shadow-lg ring-4 ring-gray-800 transition-transform hover:scale-105">
                            {profile.name?.charAt(0) || 'U'}
                        </div>
                        <div>
                            <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight">Hello, {profile.name?.split(' ')[0]}! 👋</h1>
                            <p className="text-gray-400 text-sm md:text-base font-medium">Let's keep the streak alive.</p>
                        </div>
                    </div>
                    <div className="flex flex-row md:flex-row gap-3 w-full md:w-auto">
                        <Link href="/connect" className="flex-1 md:flex-none text-center bg-gray-800 hover:bg-gray-700 text-white px-5 py-3 rounded-xl text-sm font-bold transition border border-gray-700 shadow-lg">
                            Accounts
                        </Link>
                        <button onClick={handleLogout} className="flex-1 md:flex-none bg-red-500/10 text-red-500 px-5 py-3 rounded-xl hover:bg-red-500/20 transition text-sm font-bold border border-red-500/20">
                            Logout
                        </button>
                    </div>
                </div>
